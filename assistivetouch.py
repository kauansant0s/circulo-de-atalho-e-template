...
class FirebaseAuth:
    ...
    def add_template(self, nome, texto, atalho, usuario_id, setor, compartilhado=False):
        data = {"fields": {
            "nome":          {"stringValue": nome},
            ...
            "setor":         {"stringValue": setor},
            "compartilhado": {"booleanValue": compartilhado},
        }}
    ...
    def add_atalho(self, titulo, comando_tipo, comando_valor, acoes, usuario_id, setor, compartilhado=False):
        data = {"fields": {
    ...
    def update_atalho(self, doc_id, titulo, comando_tipo, comando_valor, acoes, compartilhado=False):
        data = {"fields": {
            "titulo":        {"stringValue": titulo},
            "comando_tipo":  {"stringValue": comando_tipo},
        ...
        mask = "updateMask.fieldPaths=titulo&updateMask.fieldPaths=comando_tipo&updateMask.fieldPaths=comando_valor&updateMask.fieldPaths=acoes&updateMask.fieldPaths=compartilhado"
    ...
    def update_template(self, doc_id, nome, texto, atalho, compartilhado=False):
        data = {"fields": {
            "nome":          {"stringValue": nome},
            "texto":         {"stringValue": texto},
            "atalho":        {"stringValue": atalho or ""},
    ...
    def get_templates_setor(self, setor):
        key = ('setor', setor)
        if key not in self._cache_templates:
            self._cache_templates[key] = self._query_templates_setor_compartilhados(setor)
        return self._cache_templates[key]

    def _query_templates_setor_compartilhados(self, setor):
        """Retorna templates do setor que foram compartilhados."""
            ...
            "structuredQuery": {
                ...
                "where": {
                    "compositeFilter": {
                        "op": "AND",
                        "filters": [
                            {"fieldFilter": {"field": {"fieldPath": "setor"}, "op": "EQUAL", "value": {"stringValue": setor}}},
                            {"fieldFilter": {"field": {"fieldPath": "compartilhado"}, "op": "EQUAL", "value": {"booleanValue": True}}},
                        ]
                    }
                }
            }
        ...
        for item in resp.json():
            ...
            results.append({
                'id': doc_id, 'nome': f.get('nome', ''), 'texto': f.get('texto', ''),
                'atalho': f.get('atalho', ''), 'usuario_id': f.get('usuario_id', ''),
                'setor': f.get('setor', ''), 'compartilhado': f.get('compartilhado', False),
            })
        return results
    ...
    def _query_templates(self, field, value):
        ...
        for item in resp.json():
            ...
            results.append({
                ...
                'setor':         f.get('setor', ''),
                'compartilhado': f.get('compartilhado', False),
            })
        return results
    ...
    def get_atalhos_setor(self, setor):
        ...
        if key not in self._cache_shortcuts:
            self._cache_shortcuts[key] = self._query_atalhos_setor_compartilhados(setor)
        return self._cache_shortcuts[key]

    def _query_atalhos_setor_compartilhados(self, setor):
        """Retorna atalhos do setor que foram compartilhados."""
        url = f"https://firestore.googleapis.com/v1/projects/{self.project_id}/databases/(default)/documents:runQuery"
        body = {
            "structuredQuery": {
                "from": [{"collectionId": "atalhos"}],
                "where": {
                    "compositeFilter": {
                        "op": "AND",
                        "filters": [
        ...
        for item in resp.json():
            ...
            results.append({
                ...
                'compartilhado': f.get('compartilhado', {}).get('booleanValue', True),
            })
        return results
    ...
    def _query_atalhos(self, field, value):
        ...
        for item in resp.json():
            ...
            results.append({
                ...
                'usuario_id':    f.get('usuario_id',    {}).get('stringValue', ''),
                'setor':         f.get('setor',         {}).get('stringValue', ''),
                'compartilhado': f.get('compartilhado', {}).get('booleanValue', False),
            })
        return results
            ...
            try:
                ...
                for acao in acoes:
                    ...
                    if tipo == 'click':
                        ...
                        x1, y1 = acao.get('x1'), acao.get('y1')
                        x2, y2 = acao.get('x2'), acao.get('y2')
                        ...
                        if tecla == 'seta':
                            seta_map = {'Cima': Key.up, 'Baixo': Key.down, 'Esquerda': Key.left, 'Direita': Key.right}
                            k = seta_map.get(acao.get('seta', ''))
                            if k:
                                for _ in range(acao.get('qtd', 1)):
                                    self.keyboard_controller.press(k)
                                    self.keyboard_controller.release(k)
                                    time.sleep(0.03)
                        elif tecla == 'backspace':
                            self.keyboard_controller.press(Key.backspace); self.keyboard_controller.release(Key.backspace)
                        elif tecla == 'delete':
                            self.keyboard_controller.press(Key.delete); self.keyboard_controller.release(Key.delete)
                        elif tecla == 'tab':
                            self.keyboard_controller.press(Key.tab); self.keyboard_controller.release(Key.tab)
                        elif tecla == 'ctrl+c':
                            with self.keyboard_controller.pressed(Key.ctrl): self.keyboard_controller.press('c'); self.keyboard_controller.release('c')
                        elif tecla == 'ctrl+v':
                            with self.keyboard_controller.pressed(Key.ctrl): self.keyboard_controller.press('v'); self.keyboard_controller.release('v')
                        elif tecla == 'ctrl+x':
                            with self.keyboard_controller.pressed(Key.ctrl): self.keyboard_controller.press('x'); self.keyboard_controller.release('x')
                        texto = acao.get('texto', '')
                        if texto:
                            self._apagar_e_digitar(texto, 0)
                            time.sleep(0.1 + len(texto) * 0.01)
                    elif tipo == 'template':
                        texto = acao.get('texto', '')
                        if texto:
                            self._apagar_e_digitar(texto, 0)
...
<path d="M3.33333 12.6667H4.28333L10.8 6.15L9.85 5.2L3.33333 11.7167V12.6667ZM2 14V11.1667L10.8 2.38333C10.9333 2.26111 11.0806 2.16667 11.2417 2.1C11.4028 2.03333 11.5722 2 11.75 2C11.9278 2 12.1 2.03333 12.2667 2.1C12.4333 2.16667 12.5778 2.26667 12.7 2.4L13.6167 3.33333C13.75 3.45556 13.8472 3.6 13.9083 3.76667C13.9694 3.93333 14 4.1 14 4.26667C14 4.44444 13.9694 4.61389 13.9083 4.775C13.8472 4.93611 13.75 5.08333 13.6167 5.21667L4.83333 14H2ZM10.3167 5.68333L9.85 5.2L10.8 6.15L10.3167 5.68333Z" fill="#1D1B20"/></svg>"""
    ...
    def show_add_overlay(self):
        ...
        header_l = QHBoxLayout(header_w); header_l.setContentsMargins(0,0,0,10); header_l.setSpacing(8)
        title = QLabel("Criação de template")
        title.setStyleSheet("font-family:'Inter'; font-size:16px; font-weight:bold; color:black;")
        self._tpl_compartilhar = QCheckBox("Compartilhar com o setor?")
        self._tpl_compartilhar.setStyleSheet("QCheckBox{font-family:'Inter';font-size:12px;color:#1D1B20;background:transparent;}QCheckBox::indicator{width:15px;height:15px;border:2px solid #1D1B20;border-radius:3px;background:white;}QCheckBox::indicator:checked{background:#499714;border:2px solid #499714;}QCheckBox::indicator:hover{border:2px solid #499714;}")
        header_l.addWidget(title); header_l.addStretch(); header_l.addWidget(self._tpl_compartilhar)
        self.overlay_widget.add_content(header_w)
    ...
    def create_template(self):
        ...
        ok = self.firebase.add_template(titulo, conteudo, atalho,
                                         ...
                                         self._tpl_compartilhar.isChecked())
        if ok:
    ...
    def show_add_atalho_overlay(self, atl_existente=None):
        ...
        header_w = QWidget(); header_w.setStyleSheet("background:transparent;")
        header_l = QHBoxLayout(header_w); header_l.setContentsMargins(0,0,0,4); header_l.setSpacing(8)
        title = QLabel("Editar Atalho" if editando else "Criação de Atalho")
        title.setStyleSheet("font-family:'Inter'; font-size:14px; font-weight:bold; color:black;")
        self._atl_compartilhar = QCheckBox("Compartilhar com o setor?")
        self._atl_compartilhar.setStyleSheet("QCheckBox{font-family:'Inter';font-size:12px;color:#1D1B20;background:transparent;}QCheckBox::indicator{width:15px;height:15px;border:2px solid #1D1B20;border-radius:3px;background:white;}QCheckBox::indicator:checked{background:#499714;border:2px solid #499714;}QCheckBox::indicator:hover{border:2px solid #499714;}")
        if editando:
            self._atl_compartilhar.setChecked(atl_existente.get('compartilhado', False))
        header_l.addWidget(title); header_l.addStretch(); header_l.addWidget(self._atl_compartilhar)
        self.overlay_widget.add_content(header_w)
...
<path d="M2.5 5L7 9.5L11.5 5" stroke="#1E1E1E" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        acoes_frame.setFixedHeight(200)
        acoes_frame_layout = QVBoxLayout(acoes_frame)
        acoes_frame_layout.setContentsMargins(6, 6, 6, 6); acoes_frame_layout.setSpacing(0)

        acoes_scroll = QScrollArea()
        acoes_scroll.setWidgetResizable(True)
        acoes_scroll.setFrameShape(QFrame.Shape.NoFrame)
        acoes_scroll.setStyleSheet("""
            QScrollArea { background:transparent; border:none; }
            QScrollBar:vertical { background:#D4D3C8; width:6px; border-radius:3px; }
            QScrollBar::handle:vertical { background:#909090; border-radius:3px; min-height:20px; }
            QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height:0px; }
        """)

        acoes_inner = QWidget(); acoes_inner.setStyleSheet("background:transparent;")
        acoes_layout = QVBoxLayout(acoes_inner)
        acoes_layout.setContentsMargins(2, 2, 2, 2); acoes_layout.setSpacing(6)
        ...
        def _mostrar_editor_esperar(wrapper_existente=None):
            ...
            def _confirmar_esperar():
                ...
                coment_salvo = _pegar_comentario(wrapper_existente)
                ...
                _aplicar_comentario(card_conf, coment_salvo)
        ...
        def _criar_card_esperar(ms):
            ...
            card = _montar_card(f"Esperar {ms} ms", btn_ed, btn_rm, wrapper)
            wl.addWidget(card)
            return wrapper

        btn_esperar.clicked.connect(_iniciar_esperar)

        btn_arrastar = QPushButton("Arrastar com o mouse")
        btn_arrastar.setStyleSheet("""
            QPushButton { font-family:'Inter'; font-size:12px; color:#1D1B20; background:transparent;
                          border:none; text-align:left; padding:5px 6px; border-radius:4px; }
            QPushButton:hover { background:rgba(0,0,0,0.08); }
        """)
        btn_arrastar.setCursor(Qt.CursorShape.PointingHandCursor)
        add_acao_opts.addWidget(btn_arrastar)

        def _iniciar_captura_arraste():
            self._add_acao_expanded = False
            self._add_acao_container.setVisible(False)
            self.hide()
            self._capturar_ponto_arraste(wrapper_existente=None)

        def _capturar_ponto_arraste(wrapper_existente=None):
            screen = QApplication.primaryScreen().geometry()
            ...
            def _paint_drag(ev):
                ...
                p.setRenderHint(QPainter.RenderHint.Antialiasing)
                p.fillRect(self._capture_overlay.rect(), QColor(0, 0, 0, 80))
                if origem[0] is not None:
                    # linha verde do origem ao cursor
                    from PyQt6.QtGui import QPen
                    pen = QPen(QColor("#88C22B"), 2, Qt.PenStyle.DashLine)
                    p.setPen(pen)
                    p.drawLine(origem[0], origem[1], cursor_pos[0], cursor_pos[1])
                    # marcador origem
                    p.setBrush(QColor("#88C22B")); p.setPen(Qt.PenStyle.NoPen)
                    p.drawEllipse(origem[0] - 6, origem[1] - 6, 12, 12)
                    # marcador cursor
                    p.setBrush(QColor("white")); p.setPen(Qt.PenStyle.NoPen)
                    p.drawEllipse(cursor_pos[0] - 5, cursor_pos[1] - 5, 10, 10)
                p.end()
            self._capture_overlay.paintEvent = _paint_drag

            def _on_move(ev):
                pos = ev.position().toPoint()
                cursor_pos[0] = pos.x(); cursor_pos[1] = pos.y()
                if origem[0] is not None:
                    self._capture_overlay.update()
            self._capture_overlay.mouseMoveEvent = _on_move

            lbl_drag = QLabel("Clique, arraste e solte para definir o arraste", self._capture_overlay)
            lbl_drag.setStyleSheet("color:white; font-family:'Inter'; font-size:14px; background:transparent;")
            lbl_drag.adjustSize(); lbl_drag.move(screen.width()//2 - lbl_drag.width()//2, 30)

            def _on_press(ev):
                gpos = ev.globalPosition().toPoint()
                origem[0] = gpos.x(); origem[1] = gpos.y()
                cursor_pos[0] = gpos.x(); cursor_pos[1] = gpos.y()
                self._capture_overlay.update()
            def _on_release(ev):
                gpos = ev.globalPosition().toPoint()
                x1, y1 = origem[0], origem[1]
                x2, y2 = gpos.x(), gpos.y()
                self._capture_overlay.close(); self._capture_overlay = None
                self.show(); self.raise_()
                card = _criar_card_arraste(x1, y1, x2, y2)
                if wrapper_existente:
                    idx = self._acoes_lista_layout.indexOf(wrapper_existente)
                    wrapper_existente.setParent(None); wrapper_existente.deleteLater()
                    self._acoes_lista_layout.insertWidget(max(0, idx), card)
                else:
                    self._acoes_lista_layout.addWidget(card)
                self._notification = NotificationWidget('📍 Arraste capturado!')
                self._notification.show()

            self._capture_overlay.mousePressEvent  = _on_press
            self._capture_overlay.mouseReleaseEvent = _on_release
            self._capture_overlay.setCursor(Qt.CursorShape.CrossCursor)
            self._capture_overlay.show()

        self._capturar_ponto_arraste = _capturar_ponto_arraste

        def _criar_card_arraste(x1, y1, x2, y2, wrapper_existente=None):
            wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
            wrapper.setProperty('acao_data', json.dumps({'tipo': 'arraste', 'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2}))
            wl = QHBoxLayout(wrapper); wl.setContentsMargins(0,0,0,0); wl.setSpacing(4)
            btn_ed = QPushButton(); btn_ed.setIcon(create_svg_icon(svg_edit_a, 14)); btn_ed.setIconSize(QSize(14,14)); btn_ed.setFixedSize(22,22)
            btn_ed.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(0,0,0,0.06);border-radius:4px;}")
            btn_rm = QPushButton(); btn_rm.setIcon(create_svg_icon(svg_del_a, 14)); btn_rm.setIconSize(QSize(14,14)); btn_rm.setFixedSize(22,22)
            btn_rm.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(144,11,9,0.08);border-radius:4px;}")
            btn_ed.clicked.connect(lambda: (self.hide(), _capturar_ponto_arraste(wrapper_existente=wrapper)))
            btn_rm.clicked.connect(lambda: (wrapper.setParent(None), wrapper.deleteLater()))
            card = _montar_card("Arrastar mouse", btn_ed, btn_rm, wrapper)
            wl.addWidget(card)
            return wrapper

        btn_arrastar.clicked.connect(_iniciar_captura_arraste)
        ...
        btn_digitar = QPushButton("Digitar texto")
        btn_digitar.setStyleSheet(_btn_style)
        btn_digitar.setCursor(Qt.CursorShape.PointingHandCursor)
        add_acao_opts.addWidget(btn_digitar)

        btn_usar_tpl = QPushButton("Usar template")
        btn_usar_tpl.setStyleSheet(_btn_style)
        btn_usar_tpl.setCursor(Qt.CursorShape.PointingHandCursor)
        add_acao_opts.addWidget(btn_usar_tpl)

        def _iniciar_digitar():
            self._add_acao_expanded = False
            self._add_acao_container.setVisible(False)
            _mostrar_editor_digitar()

        def _mostrar_editor_digitar(wrapper_existente=None, texto_inicial=''):
            wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
            wl = QVBoxLayout(wrapper); wl.setContentsMargins(0,0,0,2); wl.setSpacing(2)
            ...
            lbl_pre = QLabel("Digitar texto:")
            lbl_pre.setStyleSheet("font-family:'Inter'; font-size:12px; color:#1D1B20; background:transparent; border:none;")

            txt_edit = QTextEdit()
            txt_edit.setPlaceholderText("Digite o texto que será digitado...")
            txt_edit.setFixedHeight(60)
            txt_edit.setStyleSheet("QTextEdit{font-family:'Inter';font-size:12px;color:#1D1B20;background:white;border:1px solid #aaa;border-radius:4px;padding:3px;}")
            if texto_inicial:
                txt_edit.setPlainText(texto_inicial)

            btns_row = QHBoxLayout(); btns_row.setContentsMargins(0,0,0,0); btns_row.setSpacing(4)
            btns_row.addStretch()
            btn_ok_d = QPushButton(); btn_ok_d.setIcon(create_svg_icon(svg_check, 13)); btn_ok_d.setIconSize(QSize(13,13)); btn_ok_d.setFixedSize(22,22)
            btn_ok_d.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(73,151,20,0.1);border-radius:4px;}")
            btn_cx_d = QPushButton(); btn_cx_d.setIcon(create_svg_icon(svg_xmark, 13)); btn_cx_d.setIconSize(QSize(13,13)); btn_cx_d.setFixedSize(22,22)
            btn_cx_d.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(144,11,9,0.1);border-radius:4px;}")
            btns_row.addWidget(btn_ok_d); btns_row.addWidget(btn_cx_d)

            cl.addWidget(lbl_pre); cl.addWidget(txt_edit)
            bw = QWidget(); bw.setStyleSheet("background:transparent;"); bw.setLayout(btns_row)
            cl.addWidget(bw)
            wl.addWidget(card)

            def _confirmar_digitar():
                texto = txt_edit.toPlainText().strip()
                if not texto: return
                coment_salvo = _pegar_comentario(wrapper_existente)
                card_conf = _criar_card_digitar(texto)
                idx = self._acoes_lista_layout.indexOf(wrapper)
                wrapper.setParent(None); wrapper.deleteLater()
                if wrapper_existente:
                    try: wrapper_existente.setParent(None); wrapper_existente.deleteLater()
            ...
            def _cancelar_digitar():
                if wrapper_existente: wrapper_existente.setVisible(True)
                wrapper.setParent(None); wrapper.deleteLater()

            btn_ok_d.clicked.connect(_confirmar_digitar)
            btn_cx_d.clicked.connect(_cancelar_digitar)

            def _key_txt(ev):
                if ev.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter) and not (ev.modifiers() & Qt.KeyboardModifier.ShiftModifier):
                    _confirmar_digitar()
                else:
                    QTextEdit.keyPressEvent(txt_edit, ev)
            txt_edit.keyPressEvent = _key_txt
            ...
            if wrapper_existente:
                ...
                self._acoes_lista_layout.addWidget(wrapper)
            QTimer.singleShot(50, txt_edit.setFocus)

        def _criar_card_digitar(texto):
            wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
            wrapper.setProperty('acao_data', json.dumps({'tipo': 'digitar', 'texto': texto}))
            wl = QHBoxLayout(wrapper); wl.setContentsMargins(0,0,0,0); wl.setSpacing(4)
            btn_ed = QPushButton(); btn_ed.setIcon(create_svg_icon(svg_edit_a, 14)); btn_ed.setIconSize(QSize(14,14)); btn_ed.setFixedSize(22,22)
            btn_ed.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(0,0,0,0.06);border-radius:4px;}")
            btn_rm = QPushButton(); btn_rm.setIcon(create_svg_icon(svg_del_a, 14)); btn_rm.setIconSize(QSize(14,14)); btn_rm.setFixedSize(22,22)
            btn_rm.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(144,11,9,0.08);border-radius:4px;}")
            btn_ed.clicked.connect(lambda: _mostrar_editor_digitar(wrapper, texto))
            btn_rm.clicked.connect(lambda: (wrapper.setParent(None), wrapper.deleteLater()))
            preview = texto[:30] + ('…' if len(texto) > 30 else '')
            card = _montar_card(f'Digitar: "{preview}"', btn_ed, btn_rm, wrapper)
            wl.addWidget(card)
            return wrapper

        # ── Usar template ──────────────────────────────────────────────────────
        def _iniciar_usar_template():
            self._add_acao_expanded = False
            self._add_acao_container.setVisible(False)
            _mostrar_editor_usar_template()

        def _mostrar_editor_usar_template(wrapper_existente=None, tpl_id_inicial=None):
            templates = self.firebase.get_templates_setor(self.user_data['setor'])
            wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
            wl = QVBoxLayout(wrapper); wl.setContentsMargins(0,0,0,2); wl.setSpacing(2)
            ...
            lbl_pre = QLabel("Usar template:")
            lbl_pre.setStyleSheet("font-family:'Inter'; font-size:12px; color:#1D1B20; background:transparent; border:none;")

            tpl_sel = [None]
            btn_tpl_dd = QPushButton("Selecione..." if not tpl_id_inicial else next((t['nome'] for t in templates if t['id'] == tpl_id_inicial), "Selecione..."))
            btn_tpl_dd.setFixedHeight(26); btn_tpl_dd.setMinimumWidth(120)
            btn_tpl_dd.setStyleSheet("QPushButton{font-family:'Inter';font-size:12px;color:#1D1B20;background:white;border:1px solid #aaa;border-radius:4px;padding:0 8px;}QPushButton:hover{background:#eee;}")
            btn_tpl_dd.setCursor(Qt.CursorShape.PointingHandCursor)
            if tpl_id_inicial:
                tpl_sel[0] = tpl_id_inicial

            tpl_cont = QWidget(); tpl_cont.setStyleSheet("QWidget{background:#BAB9AD;border-radius:6px;}")
            tpl_cont.setVisible(False)
            tpl_cont_l = QVBoxLayout(tpl_cont); tpl_cont_l.setContentsMargins(6,4,6,4); tpl_cont_l.setSpacing(2)

            for t in templates:
                b = QPushButton(t['nome'])
                ...
                def _sel_tpl(_, tid=t['id'], tnome=t['nome'], ttexto=t.get('texto','')):
                    tpl_sel[0] = tid; btn_tpl_dd.setText(tnome); tpl_cont.setVisible(False)
                    # salvar direto
                    coment_salvo = _pegar_comentario(wrapper_existente)
                    card_conf = _criar_card_template(tid, tnome, ttexto)
                    idx = self._acoes_lista_layout.indexOf(wrapper)
                    wrapper.setParent(None); wrapper.deleteLater()
                    if wrapper_existente:
                        try: wrapper_existente.setParent(None); wrapper_existente.deleteLater()
                    ...
                    self._acoes_lista_layout.insertWidget(max(0, idx), card_conf)
                    _aplicar_comentario(card_conf, coment_salvo)
                b.clicked.connect(_sel_tpl)
                tpl_cont_l.addWidget(b)

            btn_tpl_dd.clicked.connect(lambda: tpl_cont.setVisible(not tpl_cont.isVisible()))

            btn_ok_t = QPushButton(); btn_ok_t.setIcon(create_svg_icon(svg_check, 13)); btn_ok_t.setIconSize(QSize(13,13)); btn_ok_t.setFixedSize(22,22)
            btn_ok_t.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(73,151,20,0.1);border-radius:4px;}")
            btn_cx_t = QPushButton(); btn_cx_t.setIcon(create_svg_icon(svg_xmark, 13)); btn_cx_t.setIconSize(QSize(13,13)); btn_cx_t.setFixedSize(22,22)
            btn_cx_t.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(144,11,9,0.1);border-radius:4px;}")

            def _confirmar_template():
                tid = tpl_sel[0]
                if not tid: return
                nome = btn_tpl_dd.text()
                texto = next((t['texto'] for t in templates if t['id'] == tid), '')
                coment_salvo = _pegar_comentario(wrapper_existente)
                card_conf = _criar_card_template(tid, nome, texto)
                idx = self._acoes_lista_layout.indexOf(wrapper)
                wrapper.setParent(None); wrapper.deleteLater()
                if wrapper_existente:
                    try: wrapper_existente.setParent(None); wrapper_existente.deleteLater()
            ...
            def _cancelar_template():
                if wrapper_existente: wrapper_existente.setVisible(True)
                wrapper.setParent(None); wrapper.deleteLater()

            btn_ok_t.clicked.connect(_confirmar_template)
            btn_cx_t.clicked.connect(_cancelar_template)

            cl.addWidget(lbl_pre); cl.addWidget(btn_tpl_dd); cl.addStretch()
            cl.addWidget(btn_ok_t); cl.addWidget(btn_cx_t)
            wl.addWidget(card); wl.addWidget(tpl_cont)
        ...
        def _criar_card_template(tid, nome, texto):
            wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
            wrapper.setProperty('acao_data', json.dumps({'tipo': 'template', 'tpl_id': tid, 'nome': nome, 'texto': texto}))
            wl = QHBoxLayout(wrapper); wl.setContentsMargins(0,0,0,0); wl.setSpacing(4)
            btn_ed = QPushButton(); btn_ed.setIcon(create_svg_icon(svg_edit_a, 14)); btn_ed.setIconSize(QSize(14,14)); btn_ed.setFixedSize(22,22)
            btn_ed.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(0,0,0,0.06);border-radius:4px;}")
            btn_rm = QPushButton(); btn_rm.setIcon(create_svg_icon(svg_del_a, 14)); btn_rm.setIconSize(QSize(14,14)); btn_rm.setFixedSize(22,22)
            btn_rm.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(144,11,9,0.08);border-radius:4px;}")
            btn_ed.clicked.connect(lambda: _mostrar_editor_usar_template(wrapper, tid))
            btn_rm.clicked.connect(lambda: (wrapper.setParent(None), wrapper.deleteLater()))
            card = _montar_card(f'Template: "{nome}"', btn_ed, btn_rm, wrapper)
            wl.addWidget(card)
            return wrapper

        btn_digitar.clicked.connect(_iniciar_digitar)
        btn_usar_tpl.clicked.connect(_iniciar_usar_template)

        btn_teclas = QPushButton("Comandos / teclas especiais")
        btn_teclas.setStyleSheet(_btn_style)
        btn_teclas.setCursor(Qt.CursorShape.PointingHandCursor)
        add_acao_opts.addWidget(btn_teclas)

        def _iniciar_teclas():
            self._add_acao_expanded = False
            self._add_acao_container.setVisible(False)
            _mostrar_editor_teclas()
        ...
        def _mostrar_editor_teclas(wrapper_existente=None, tecla_inicial=None, qtd_inicial=1, seta_inicial=None):

            def _salvar_tecla(k, nome_card, dados):
                coment_salvo = _pegar_comentario(wrapper_existente)
                card_conf = _criar_card_tecla(nome_card, dados)
                if wrapper_existente:
                    idx = self._acoes_lista_layout.indexOf(wrapper_existente)
                    wrapper_existente.setParent(None); wrapper_existente.deleteLater()
                    self._acoes_lista_layout.insertWidget(max(0, idx), card_conf)
                else:
                    self._acoes_lista_layout.addWidget(card_conf)
                _aplicar_comentario(card_conf, coment_salvo)

            # se editando e não é seta → mostrar dropdown direto
            # sempre montar o editor de setas inline quando seta
            if tecla_inicial == 'seta' or tecla_inicial is None:
                # montar card editor de setas
                wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
                wl = QVBoxLayout(wrapper); wl.setContentsMargins(0,0,0,2); wl.setSpacing(2)
                card = QWidget(); card.setStyleSheet("background:#C2C0B6; border-radius:6px;")
                cl = QHBoxLayout(card); cl.setContentsMargins(8,6,8,6); cl.setSpacing(4)

                lbl_pre = QLabel("Tecla / comando:")
                lbl_pre.setStyleSheet("font-family:'Inter'; font-size:12px; color:#1D1B20; background:transparent; border:none;")

                tecla_cont = QWidget(); tecla_cont.setStyleSheet("QWidget{background:#BAB9AD;border-radius:6px;}")
                tecla_cont_l = QVBoxLayout(tecla_cont); tecla_cont_l.setContentsMargins(6,4,6,4); tecla_cont_l.setSpacing(2)

                # se não é seta, mostrar todas as opções no dropdown
                if tecla_inicial is None:
                    for nome, key in TECLAS_SIMPLES:
                        b = QPushButton(nome)
                        b.setStyleSheet("QPushButton{font-family:'Inter';font-size:12px;color:#1D1B20;background:transparent;border:none;text-align:left;padding:4px 6px;border-radius:4px;}QPushButton:hover{background:rgba(0,0,0,0.08);}")
                        b.setCursor(Qt.CursorShape.PointingHandCursor)
                        def _sel_simples(_, n=nome, k=key):
                            wrapper.setParent(None); wrapper.deleteLater()
                            _salvar_tecla(k, n, {'tipo': 'tecla', 'tecla': k})
                        b.clicked.connect(_sel_simples)
                        tecla_cont_l.addWidget(b)
                    b_seta = QPushButton("Setas ›")
                    b_seta.setStyleSheet("QPushButton{font-family:'Inter';font-size:12px;color:#1D1B20;background:transparent;border:none;text-align:left;padding:4px 6px;border-radius:4px;}QPushButton:hover{background:rgba(0,0,0,0.08);}")
                    b_seta.setCursor(Qt.CursorShape.PointingHandCursor)
                    def _ir_setas():
                        wrapper.setParent(None); wrapper.deleteLater()
                        _mostrar_editor_teclas(wrapper_existente, 'seta')
                    b_seta.clicked.connect(_ir_setas)
                    tecla_cont_l.addWidget(b_seta)

                    cl.addWidget(lbl_pre); cl.addStretch()
                    wl.addWidget(card); wl.addWidget(tecla_cont)
                    tecla_cont.setVisible(True)
                ...
                # ── editor de setas (igual ao mouse: dd com setas + campo qtd) ──
                seta_sel = [seta_inicial]
                idx_hover = [-1]
                direcoes  = ['Cima', 'Baixo', 'Esquerda', 'Direita']

                btn_dd = QPushButton(f'Seta {seta_inicial}' if seta_inicial else '▾')
                btn_dd.setFixedHeight(26); btn_dd.setMinimumWidth(110)
                ...
                if tecla_inicial == 'seta' and seta_inicial:
                    spin.setText(str(qtd_inicial)); _upd_vez()
                    qtd_w.setVisible(True)
                else:
                ...
                for d in direcoes:
                    b = QPushButton(f'Seta {d}')
                    b.setStyleSheet("QPushButton{font-family:'Inter';font-size:12px;color:#1D1B20;background:transparent;border:none;text-align:left;padding:4px 6px;border-radius:4px;}QPushButton:hover{background:rgba(0,0,0,0.08);}")
                    b.setCursor(Qt.CursorShape.PointingHandCursor)
                    def _sel(_, _d=d):
                        seta_sel[0] = _d; btn_dd.setText(f'Seta {_d}')
                        dd_cont.setVisible(False); qtd_w.setVisible(True)
                        QTimer.singleShot(50, spin.setFocus)
                    b.clicked.connect(_sel)
                    dc_l.addWidget(b); botoes_dd.append(b)
                ...
                def _confirmar_seta():
                    ...
                    wrapper.setParent(None); wrapper.deleteLater()
                    _salvar_tecla('seta', f'Seta {d} {qtd} {"vez" if qtd == 1 else "vezes"}', {'tipo': 'tecla', 'tecla': 'seta', 'seta': d, 'qtd': qtd})
                ...
                def _key_dd(ev):
                    if ev.key() == Qt.Key.Key_Up:
                        idx_hover[0] = max(0, idx_hover[0] - 1); _atualizar_hl()
                    elif ev.key() == Qt.Key.Key_Down:
                        idx_hover[0] = min(len(direcoes)-1, idx_hover[0] + 1); _atualizar_hl()
                    elif ev.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
                        if 0 <= idx_hover[0] < len(direcoes):
                            d = direcoes[idx_hover[0]]
                            seta_sel[0] = d; btn_dd.setText(f'Seta {d}')
                            dd_cont.setVisible(False); qtd_w.setVisible(True)
                            QTimer.singleShot(50, spin.setFocus)
                    elif ev.key() == Qt.Key.Key_Escape:
                        _cancelar_seta()
                    else:
                        QPushButton.keyPressEvent(btn_dd, ev)
                btn_dd.keyPressEvent = _key_dd
                btn_dd.clicked.connect(lambda: (dd_cont.setVisible(not dd_cont.isVisible()), _atualizar_hl() if dd_cont.isVisible() else None))

                def _key_spin(ev):
                    if ev.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
                        _confirmar_seta()
                    else:
                        QLineEdit.keyPressEvent(spin, ev)
                spin.keyPressEvent = _key_spin

                btn_ok.clicked.connect(_confirmar_seta)
                btn_cx.clicked.connect(_cancelar_seta)

                cl.addWidget(lbl_pre); cl.addWidget(btn_dd); cl.addWidget(qtd_w); cl.addStretch()
                cl.addWidget(btn_ok); cl.addWidget(btn_cx)
                wl.addWidget(card); wl.addWidget(dd_cont)

                if wrapper_existente:
                    wrapper_existente.setVisible(False)
                    idx2 = self._acoes_lista_layout.indexOf(wrapper_existente)
                    self._acoes_lista_layout.insertWidget(idx2, wrapper)
                else:
                    self._acoes_lista_layout.addWidget(wrapper)

                # abrir dropdown automaticamente se novo (não editando)
                if not seta_inicial:
                    def _init_dd():
                        idx_hover[0] = 0; _atualizar_hl()
                        dd_cont.setVisible(True)
                        self.activateWindow(); self.raise_()
                        btn_dd.setFocus(Qt.FocusReason.OtherFocusReason)
                    QTimer.singleShot(100, _init_dd)

            else:
                # tecla simples editando: só salvar direto
                nome_card = next((n for n, k in TECLAS_SIMPLES if k == tecla_inicial), tecla_inicial)
                _salvar_tecla(tecla_inicial, nome_card, {'tipo': 'tecla', 'tecla': tecla_inicial})

        def _criar_card_tecla(nome_card, dados):
            wrapper = QWidget(); wrapper.setStyleSheet("background:transparent;")
            wrapper.setProperty('acao_data', json.dumps(dados))
            wl = QHBoxLayout(wrapper); wl.setContentsMargins(0,0,0,0); wl.setSpacing(4)
            btn_ed = QPushButton(); btn_ed.setIcon(create_svg_icon(svg_edit_a, 14)); btn_ed.setIconSize(QSize(14,14)); btn_ed.setFixedSize(22,22)
            btn_ed.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(0,0,0,0.06);border-radius:4px;}")
            btn_rm = QPushButton(); btn_rm.setIcon(create_svg_icon(svg_del_a, 14)); btn_rm.setIconSize(QSize(14,14)); btn_rm.setFixedSize(22,22)
            btn_rm.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(144,11,9,0.08);border-radius:4px;}")
            btn_ed.clicked.connect(lambda: _mostrar_editor_teclas(
                wrapper, dados.get('tecla'), dados.get('qtd', 1), dados.get('seta')
            ))
            btn_rm.clicked.connect(lambda: (wrapper.setParent(None), wrapper.deleteLater()))
            card = _montar_card(nome_card, btn_ed, btn_rm, wrapper)
            wl.addWidget(card)
            return wrapper

        btn_teclas.clicked.connect(_iniciar_teclas)
        ...
        svg_del_a  = """<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2 3.99998H3.33333M3.33333 3.99998H14M3.33333 3.99998L3.33333 13.3333C3.33333 13.6869 3.47381 14.0261 3.72386 14.2761C3.97391 14.5262 4.31304 14.6666 4.66667 14.6666H11.3333C11.687 14.6666 12.0261 14.5262 12.2761 14.2761C12.5262 14.0261 12.6667 13.6869 12.6667 13.3333V3.99998M5.33333 3.99998V2.66665C5.33333 2.31302 5.47381 1.97389 5.72386 1.72384C5.97391 1.47379 6.31304 1.33331 6.66667 1.33331H9.33333C9.68696 1.33331 10.0261 1.47379 10.2761 1.72384C10.5262 1.97389 10.6667 2.31302 10.6667 2.66665V3.99998" stroke="#900B09" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>"""
        svg_edit_a = """<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.33333 12.6667H4.28333L10.8 6.15L9.85 5.2L3.33333 11.7167V12.6667ZM2 14V11.1667L10.8 2.38333C10.9333 2.26111 11.0806 2.16667 11.2417 2.1C11.4028 2.03333 11.5722 2 11.75 2C11.9278 2 12.1 2.03333 12.2667 2.1C12.4333 2.16667 12.5778 2.26667 12.7 2.4L13.6167 3.33333C13.75 3.45556 13.8472 3.6 13.9083 3.76667C13.9694 3.93333 14 4.1 14 4.26667C14 4.44444 13.9694 4.61389 13.9083 4.775C13.8472 4.93611 13.75 5.08333 13.6167 5.21667L4.83333 14H2ZM10.3167 5.68333L9.85 5.2L10.8 6.15L10.3167 5.68333Z" fill="#1D1B20"/></svg>"""
        svg_coment_a = """<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.49996 8.16669H10.5V7.00002H3.49996V8.16669ZM3.49996 6.41669H10.5V5.25002H3.49996V6.41669ZM3.49996 4.66669H10.5V3.50002H3.49996V4.66669ZM12.8333 12.8334L10.5 10.5H2.33329C2.01246 10.5 1.73781 10.3858 1.50933 10.1573C1.28086 9.92884 1.16663 9.65419 1.16663 9.33335V2.33335C1.16663 2.01252 1.28086 1.73787 1.50933 1.5094C1.73781 1.28092 2.01246 1.16669 2.33329 1.16669H11.6666C11.9875 1.16669 12.2621 1.28092 12.4906 1.5094C12.7191 1.73787 12.8333 2.01252 12.8333 2.33335V12.8334ZM2.33329 9.33335H10.9958L11.6666 9.9896V2.33335H2.33329V9.33335Z" fill="#1D1B20"/></svg>"""

        def _criar_btn_comentario(card_inner_ref):
            """Botão que adiciona/edita comentário dentro do card (abaixo da linha de topo)."""
            btn_c = QPushButton()
            btn_c.setIcon(create_svg_icon(svg_coment_a, 14))
            btn_c.setIconSize(QSize(14, 14)); btn_c.setFixedSize(22, 22)
            btn_c.setStyleSheet("QPushButton{background:transparent;border:none;}QPushButton:hover{background:rgba(0,0,0,0.06);border-radius:4px;}")
            btn_c.setCursor(Qt.CursorShape.PointingHandCursor)

            def _toggle_comentario():
                card = card_inner_ref[0]
                coment_w = card.findChild(QWidget, 'coment_widget')
                if coment_w:
                    lbl   = coment_w.findChild(QLabel,    'coment_label')
                    field = coment_w.findChild(QLineEdit, 'coment_field')
                    if field and field.isVisible():
                        # está em modo edição — cancelar: voltar ao label se tiver texto, senão remover
                        field.setVisible(False)
                        if lbl and lbl.text().strip():
                            lbl.setVisible(True)
                        else:
                            coment_w.setParent(None); coment_w.deleteLater()
                    elif lbl and lbl.isVisible():
                        # está mostrando label — abrir edição
                        lbl.setVisible(False)
                        field.setText(lbl.text()); field.setVisible(True)
                        QTimer.singleShot(50, field.setFocus)
                    return
                # criar pela primeira vez
                coment_w = QWidget(); coment_w.setObjectName('coment_widget')
                coment_w.setStyleSheet("background:transparent;")
                cl2 = QHBoxLayout(coment_w); cl2.setContentsMargins(0, 0, 0, 0); cl2.setSpacing(0)
                field = QLineEdit(); field.setObjectName('coment_field')
                field.setPlaceholderText("Adicionar comentário... (Enter para salvar)")
                field.setStyleSheet("QLineEdit{font-family:'Instrument Sans';font-size:11px;color:#828282;background:transparent;border:none;border-bottom:1px solid #C2C0B6;padding:1px 0;}QLineEdit::placeholder{color:#aaa;}")
                lbl = QLabel(); lbl.setObjectName('coment_label')
                lbl.setStyleSheet("font-family:'Instrument Sans';font-size:11px;color:#828282;background:transparent;border:none;")
                lbl.setVisible(False)
                def _salvar():
                    t = field.text().strip()
                    field.setVisible(False)
                    if t: lbl.setText(t); lbl.setVisible(True)
                    else: coment_w.setVisible(False)
                field.returnPressed.connect(_salvar)
                cl2.addWidget(field); cl2.addWidget(lbl)
                card.layout().addWidget(coment_w)  # adiciona no VLayout do card, abaixo da linha topo
                QTimer.singleShot(50, field.setFocus)

            btn_c.clicked.connect(_toggle_comentario)
            return btn_c

        def _pegar_comentario(wrapper):
            """Retorna o texto do comentário atual de um card, ou ''."""
            if not wrapper: return ''
            card_inner = wrapper.findChild(QWidget, 'card_inner')
            if not card_inner: return ''
            coment_w = card_inner.findChild(QWidget, 'coment_widget')
            if not coment_w: return ''
            # não checar isVisible() pois o wrapper pode estar oculto durante edição
            lbl = coment_w.findChild(QLabel, 'coment_label')
            field = coment_w.findChild(QLineEdit, 'coment_field')
            if lbl and lbl.text().strip(): return lbl.text().strip()
            if field and field.text().strip(): return field.text().strip()
            return ''

        def _aplicar_comentario(novo_wrapper, texto):
            """Restaura comentário num card recém-criado."""
            if not texto: return
            card_inner = novo_wrapper.findChild(QWidget, 'card_inner')
            if not card_inner: return
            coment_w = QWidget(); coment_w.setObjectName('coment_widget')
            coment_w.setStyleSheet("background:transparent;")
            cl2 = QHBoxLayout(coment_w); cl2.setContentsMargins(0,0,0,0); cl2.setSpacing(0)
            ...
            field.setStyleSheet("QLineEdit{font-family:'Instrument Sans';font-size:11px;color:#828282;background:transparent;border:none;border-bottom:1px solid #C2C0B6;padding:1px 0;}QLineEdit::placeholder{color:#aaa;}")
            field.setVisible(False)
            lbl = QLabel(texto); lbl.setObjectName('coment_label')
            lbl.setStyleSheet("font-family:'Instrument Sans';font-size:11px;color:#828282;background:transparent;border:none;")
            def _salvar():
                t = field.text().strip()
                field.setVisible(False)
            ...
            field.returnPressed.connect(_salvar)
            cl2.addWidget(field); cl2.addWidget(lbl)
            card_inner.layout().addWidget(coment_w)

        def _montar_card(desc_txt, btn_ed, btn_rm, wrapper):
            """card com VLayout: linha topo (desc+botões) e espaço para comentário abaixo."""
            card = QWidget(); card.setObjectName('card_inner')
            card.setStyleSheet("background:#C2C0B6; border-radius:6px;")
            cv = QVBoxLayout(card); cv.setContentsMargins(8,6,8,6); cv.setSpacing(3)
            top = QWidget(); top.setStyleSheet("background:transparent;")
            tl = QHBoxLayout(top); tl.setContentsMargins(0,0,0,0); tl.setSpacing(6)
            desc = QLabel(desc_txt)
            desc.setStyleSheet("font-family:'Inter'; font-size:12px; color:#1D1B20; background:transparent; border:none;")
            tl.addWidget(desc); tl.addStretch()
            btn_cm = _criar_btn_comentario([card])
            tl.addWidget(btn_cm); tl.addWidget(btn_ed); tl.addWidget(btn_rm)
            cv.addWidget(top)
            return card
        ...
        def _criar_card_conf(x, y, op, qtd):
            ...
            btn_rm.clicked.connect(lambda: (wrapper.setParent(None), wrapper.deleteLater()))
            card = _montar_card(txt, btn_ed, btn_rm, wrapper)
            wl.addWidget(card)
            return wrapper
        ...
        acoes_scroll.setWidget(acoes_inner)
        acoes_frame_layout.addWidget(acoes_scroll)
        ...
        if editando:
            ...
            # pré-carregar ações
            def _restaurar_comentario(card_widget, acao):
                coment = acao.get('comentario', '')
                if not coment: return
                card_inner = card_widget.findChild(QWidget, 'card_inner')
                if not card_inner: return
                coment_w = QWidget(); coment_w.setObjectName('coment_widget')
                coment_w.setStyleSheet("background:transparent;")
                cl2 = QHBoxLayout(coment_w); cl2.setContentsMargins(0, 0, 0, 0); cl2.setSpacing(0)
                ...
                field.setStyleSheet("QLineEdit{font-family:'Instrument Sans';font-size:11px;color:#828282;background:transparent;border:none;border-bottom:1px solid #C2C0B6;padding:1px 0;}QLineEdit::placeholder{color:#aaa;}")
                field.setVisible(False)
                lbl = QLabel(coment); lbl.setObjectName('coment_label')
                lbl.setStyleSheet("font-family:'Instrument Sans';font-size:11px;color:#828282;background:transparent;border:none;")
                def _salvar():
                    t = field.text().strip()
                    field.setVisible(False)
            ...
            for acao in atl_existente.get('acoes', []):
                if isinstance(acao, dict) and acao.get('tipo') == 'click':
                    ...
                    self._acoes_lista_layout.addWidget(card)
                    _restaurar_comentario(card, acao)
                elif isinstance(acao, dict) and acao.get('tipo') == 'esperar':
                    card = _criar_card_esperar(acao['ms'])
                    self._acoes_lista_layout.addWidget(card)
                    _restaurar_comentario(card, acao)
                elif isinstance(acao, dict) and acao.get('tipo') == 'arraste':
                    card = _criar_card_arraste(acao['x1'], acao['y1'], acao['x2'], acao['y2'])
                    self._acoes_lista_layout.addWidget(card)
                    _restaurar_comentario(card, acao)
                elif isinstance(acao, dict) and acao.get('tipo') == 'digitar':
                    card = _criar_card_digitar(acao.get('texto', ''))
                    self._acoes_lista_layout.addWidget(card)
                    _restaurar_comentario(card, acao)
                elif isinstance(acao, dict) and acao.get('tipo') == 'template':
                    card = _criar_card_template(acao.get('tpl_id', ''), acao.get('nome', ''), acao.get('texto', ''))
                    self._acoes_lista_layout.addWidget(card)
                    _restaurar_comentario(card, acao)
                elif isinstance(acao, dict) and acao.get('tipo') == 'tecla':
                    if acao.get('tecla') == 'seta':
                        nome_card = f"Seta {acao.get('seta', '')} {acao.get('qtd', 1)} {'vez' if acao.get('qtd', 1) == 1 else 'vezes'}"
                    else:
                        nome_card = next((n for n, k in [('Backspace','backspace'),('Delete','delete'),('Copiar','ctrl+c'),('Colar','ctrl+v'),('Recortar','ctrl+x'),('Tab','tab')] if k == acao.get('tecla')), acao.get('tecla',''))
                    card = _criar_card_tecla(nome_card, acao)
                    self._acoes_lista_layout.addWidget(card)
                    _restaurar_comentario(card, acao)
            ...
            for i in range(layout.count()):
                ...
                if item and item.widget():
                    ...
                    if data:
                        ...
                        # incluir comentário se existir (dentro do card_inner)
                        card_inner = w.findChild(QWidget, 'card_inner')
                        coment_w = card_inner.findChild(QWidget, 'coment_widget') if card_inner else None
                        if coment_w and coment_w.isVisible():
                            lbl_c   = coment_w.findChild(QLabel,    'coment_label')
                            field_c = coment_w.findChild(QLineEdit, 'coment_field')
                            texto_c = ''
                            if lbl_c and lbl_c.isVisible() and lbl_c.text().strip():
                                texto_c = lbl_c.text().strip()
                            elif field_c and field_c.isVisible() and field_c.text().strip():
                                texto_c = field_c.text().strip()
                            if texto_c:
                                acao['comentario'] = texto_c
                        acoes.append(acao)
        if editando:
            ok = self.firebase.update_atalho(
                atl_existente['id'], titulo, cmd_tipo, cmd_valor, acoes,
                getattr(self, '_atl_compartilhar', None) and self._atl_compartilhar.isChecked()
            )
            ...
            ok = self.firebase.add_atalho(
                ...
                self.user_data['uid'], self.user_data['setor'],
                self._atl_compartilhar.isChecked()
            )
            msg = '✓ Atalho criado!'
        if ok:
            self.overlay_widget.close()
    ...
    def show_edit_overlay(self, t):
        ...
        title.setStyleSheet("font-family:'Inter'; font-size:16px; font-weight:bold; color:black;")
        self._tpl_compartilhar = QCheckBox("Compartilhar com o setor?")
        self._tpl_compartilhar.setChecked(t.get('compartilhado', False))
        self._tpl_compartilhar.setStyleSheet("QCheckBox{font-family:'Inter';font-size:12px;color:#1D1B20;background:transparent;}QCheckBox::indicator{width:15px;height:15px;border:2px solid #1D1B20;border-radius:3px;background:white;}QCheckBox::indicator:checked{background:#499714;border:2px solid #499714;}QCheckBox::indicator:hover{border:2px solid #499714;}")
        header_l.addWidget(title); header_l.addStretch(); header_l.addWidget(self._tpl_compartilhar)
        self.overlay_widget.add_content(header_w)
